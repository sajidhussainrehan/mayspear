...
async function cleanAndSeed() {
  try {
    // 1. Get all blogs on production
    const resGet = await fetch(`${API_URL}/blogs`);
    const existing = await resGet.json();
    console.log(`Found ${existing.length} blogs on production. Cleaning...`);

    // 2. Delete all
    if (Array.isArray(existing)) {
      for (const blog of existing) {
        await fetch(`${API_URL}/blogs/${blog. id}`, { method: 'DELETE' });
        console.log(`Deleted: ${blog.title}`);
      }
    }

    // 3. Seed the 2 good ones
    console.log('Seeding the 2 premium blogs...');
    for (const blog of blogsToAdd) {
      const resPost = await fetch(`${API_URL}/blogs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(blog)
      });
      const data = await resPost.json();
      console.log(`Added: ${blog.title}`, data. id);
    }
    console.log('DONE!');
  } catch (err) {
